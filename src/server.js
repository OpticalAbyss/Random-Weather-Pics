...
const app = express()
const port = 8000
const apiKey = `AIzaSyA2ca1gIfURrlwNzD7KHufdduXcoyWDQ4o`
const coutnries = [
...
const weatherAPIkey = `897ce7d6bf5d362a0c012ab80842d4a8`

const staticAbout = fs.readFileSync(path.join(__dirname, `../public/about.html`), "utf-8")
app.get('/about', (req, res)=>{
  res.send(staticAbout)
})

app.set("view engine", "hbs")

app.get('',(req, res)=>{
  const currentCount = coutnries[Math.round(Math.random()*60)].name
  requests(`https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${currentCount}&inputtype=textquery&key=${apiKey}`, {json: true})
    .on('data', (chunk)=>{
      const objData = JSON.parse(chunk);
      const placeid = objData.candidates[0].place_id
      requests(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeid}&key=${apiKey}`, {json: true})
